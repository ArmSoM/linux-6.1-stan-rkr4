...
build_wifibt()
{
	...
	APRKBT_DIR="$RK_SDK_DIR/kernel-6.1/drivers/bluetooth"
	...
	# if ! [[ "$RK_KERNEL_VERSION_RAW" = "6.1" ]];then
	$KMAKE M=$RKWIFIBT_DIR/tools/rtk_hciattach
	if [[ "$RK_KERNEL_VERSION_RAW" = "6.1" ]];then
	...
	if [[ "$RK_WIFIBT_MODULES" = "ALL_AP" ]];then
		...
		mv $APRKBT_DIR/hci_uart.ko $APRKBT_DIR/ap_hci_uart.ko
		cp $APRKBT_DIR/ap_hci_uart.ko $TARGET_DIR/lib/modules/
